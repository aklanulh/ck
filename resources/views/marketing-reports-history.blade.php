...
<head>
    ...
    <title>Riwayat Laporan Marketing - CatatanKerja</title>
...
<body class="bg-gray-50">
    ...
    <main class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div class="px-4 py-6 sm:px-0">
            <!-- Header -->
            <div class="mb-6">
                <h2 class="text-2xl font-bold text-gray-900">Riwayat Laporan Marketing</h2>
                <p class="text-gray-600 mt-1">Semua laporan marketing yang telah dikirim</p>
            </div>
            ...
            <!-- Reports List -->
            <div id="reportsList" class="hidden space-y-4">
            ...
            <div id="emptyState" class="hidden text-center py-12">
                ...
                <p class="text-gray-600">Anda belum memiliki riwayat laporan marketing</p>
        ...
        <div class="relative top-10 mx-auto p-5 border w-11/12 md:w-4/5 lg:w-3/4 shadow-lg rounded-md bg-white max-h-[90vh] overflow-y-auto">
            <div class="mt-3">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-medium text-gray-900">Detail Laporan Marketing</h3>
    ...
    <script>
                ...
                .then(data => {
                    ...
                    if (data.success && data.data.length > 0) {
                        ...
                        data.data.forEach(report => {
                            html += `
                                <div class="bg-white shadow rounded-lg p-6 hover:shadow-md transition-shadow cursor-pointer" onclick="viewReport(${report.id})">
                                    <div class="flex justify-between items-start">
                                        <div class="flex-1">
                                            <h3 class="text-lg font-medium text-gray-900 mb-2">${report.lokasi}</h3>
                                            <div class="flex items-center space-x-4 text-sm text-gray-600">
                                                <span><i class="fas fa-calendar mr-1"></i>${report.tanggal}</span>
                                                <span><i class="fas fa-map-marker-alt mr-1"></i>${report.total_locations} lokasi</span>
                                                <span><i class="fas fa-user mr-1"></i>${report.user_name}</span>
                                            </div>
                                        </div>
                                        <div class="ml-4">
        ...
        function openReportDetailModal(reportId) {
            ...
            // Show loading state
            content.innerHTML = '<div class="text-center py-8"><i class="fas fa-spinner fa-spin text-3xl text-blue-500"></i><p class="mt-3 text-gray-600">Memuat detail...</p></div>';
            
            modal.classList.remove('hidden');
            
            fetch(`/api/marketing-reports/${reportId}`)
                .then(response => {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        content.innerHTML = `<div class="text-center py-8 text-red-500"><i class="fas fa-exclamation-triangle text-4xl mb-3"></i><p>Gagal memuat detail laporan</p><p class="text-sm mt-2">${data.error || 'Error tidak diketahui'}</p></div>`;
                        showNotification(data.error || 'Gagal memuat detail laporan', 'error');
                    }
                })
                .catch(error => {
                    console.error('Error loading report:', error);
                    content.innerHTML = `<div class="text-center py-8 text-red-500"><i class="fas fa-exclamation-triangle text-4xl mb-3"></i><p>Gagal memuat detail laporan</p><p class="text-sm mt-2">${error.message}</p></div>`;
